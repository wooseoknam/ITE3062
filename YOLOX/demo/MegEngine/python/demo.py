...
class Predictor(object):
    ...
    def visual(self, output, img_info, temp, cls_conf=0.35):
...
def imageflow_demo(predictor, vis_folder, current_time, args, temp):
    ...
    while True:
        ...
        if ret_val:
            ...
            temp.append(outputs)
            result_frame, abcde = predictor.visual(outputs[0], img_info, temp)
            logger.info(abcde)
            if args.save_result:
...
def main(args):
    ...
    if args.demo == "image":
        ...
        imageflow_demo(predictor, vis_folder, current_time, args, temp)
        logger.info(temp)
